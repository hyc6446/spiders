...
from proweb.items import ProwebNoticeItem
settings = get_project_settings()

class TycPublicnoticeSpider(scrapy.Spider):
    '''
        公告催示
    '''
    name = 'tyc_publicnotice'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasNoticeSign","hasnotice:"+key):
                    url = "https://www.tianyancha.com/pagination/publicnoticeItem.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}#
                #公告详情
                table_dict["notice_info"] = tr.xpath("./td[last()]/script/text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/publicnoticeItem.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
