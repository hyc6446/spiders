...
from proweb.items import ProwebPurchaselandItem
settings = get_project_settings()

class TycPurchaselandSpider(scrapy.Spider):
    '''
        购地信息
    '''
    name = 'tyc_purchaseland'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasLandSign","hasland:"+key):
                    url = "https://www.tianyancha.com/pagination/purchaselandV2.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}  #
                # 购地信息
                table_dict["land_info"] = tr.xpath("./td[2]/script/text()").extract_first() if tr.xpath("./td[2]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/purchaselandV2.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num, meta['id'], meta['name'])
