...
from proweb.items import ProwebPermissionItem
settings = get_project_settings()

class TycPermissionSpider(scrapy.Spider):
    '''
        电信许可
    '''
    name = 'tyc_permission'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasPermissionSign","haspermission:"+key):
                    url = "https://www.tianyancha.com/pagination/permission.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}
                #电信许可信息
                table_dict["telecom_info"] = tr.xpath("./td[last()]/script/text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/permission.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
