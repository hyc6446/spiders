...
from proweb.items import ProwebAbnormalItem
settings = get_project_settings()

class TycAbnormalSpider(scrapy.Spider):
    '''
        经营异常
    '''
    name = 'tyc_abnormal'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasAbnormalSign","hasabnormal:"+key):
                    url = "https://www.tianyancha.com/pagination/abnormal.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}#
                #列入日期
                table_dict["join_time"] = tr.xpath("./td[2]//text()").extract_first() if tr.xpath("./td[2]//text()") else "-"
                #列入原因
                table_dict['join_cause'] = tr.xpath("./td[3]//text()").extract_first() if tr.xpath("./td[3]//text()") else "-"
                #决定机关
                table_dict['office'] = tr.xpath("./td[4]//text()").extract_first() if tr.xpath("./td[4]//text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/abnormal.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
