...
from proweb.items import ProwebBondItem
settings = get_project_settings()

class TycBondSpider(scrapy.Spider):
    '''
        债券信息
    '''
    name = 'tyc_bond'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasBondSign","hasbond:"+key):
                    url = "https://www.tianyancha.com/pagination/bond.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}  #
                # 进出口信息详情
                table_dict["bond_info"] = tr.xpath("./td[last()]/script/text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/bond.xhtml?ps=30&pn=%s&id=%s&name=%s" % (
