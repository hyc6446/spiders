...
from proweb.items import ProwebDishonestItem
settings = get_project_settings()

class DishonestSpider(scrapy.Spider):
    '''
        失信人员
    '''
    name = 'tyc_dishonest'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasDishonestSign","hasdishonest:"+key):
                    url = "https://www.tianyancha.com/pagination/dishonest.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}
                table_dict["change_time"] = tr.xpath("./td[last()]/script//text()").extract_first() if tr.xpath("./td[last()]/script//text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/dishonest.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
