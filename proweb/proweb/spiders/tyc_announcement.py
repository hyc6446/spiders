...
from proweb.items import ProwebAnnouncementItem
settings = get_project_settings()

class TycAnnouncementSpider(scrapy.Spider):
    '''
        开庭公告
    '''
    name = 'tyc_announcement'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasAnnouncementSign","hasannouncement:"+key):
                    url = "https://www.tianyancha.com/pagination/announcementcourt.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if response.text:
            ...
            for tr in table_trs:
                table_dict = {}
                table_dict["announcement_info"] = tr.xpath("./td[last()]/script//text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/announcementcourt.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
