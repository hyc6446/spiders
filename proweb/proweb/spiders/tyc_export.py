...
from proweb.items import ProwebImportAndExportItem
settings = get_project_settings()

class TycExportSpider(scrapy.Spider):
    '''
        进出口信息
    '''
    name = 'tyc_export'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasExportSign","hasexport:"+key):
                    url = "https://www.tianyancha.com/pagination/importAndExport.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        item = ProwebImportAndExportItem()
        info_list = []
        sign = None
        meta = response.meta
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}#
                #进出口信息详情
                table_dict["export_info"] = tr.xpath("./td[last()]/script/text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/importAndExport.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
