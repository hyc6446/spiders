...
from proweb.items import ProwebCopyrightItem
settings = get_project_settings()

class TycCopyrightSpider(scrapy.Spider):
    '''
        软件著作权
    '''
    name = 'tyc_copyright'
    ...
    def start_requests(self):
        ...
        while True:
            ...
            for item in allkeys:
                ...
                if not redis_pool.sismember("hasCopyrightSign","hascopyright:"+key):
                    url = "https://www.tianyancha.com/pagination/copyright.xhtml?ps=30&pn=1&id=%s&name=%s"%(key,quote_plus(val['name']))
    ...
    def parse(self, response):
        ...
        if table_trs:
            for tr in table_trs:
                table_dict = {}#
                #软件著作权详情
                table_dict["copy_info"] = tr.xpath("./td[last()]/script/text()").extract_first() if tr.xpath("./td[last()]/script/text()") else "-"
            ...
            if next_ul:
                if next_ul.css("li a.-next"):
                    ...
                    sign = True
                url = "https://www.tianyancha.com/pagination/copyright.xhtml?ps=30&pn=%s&id=%s&name=%s"%(next_num,meta['id'],meta['name'])
